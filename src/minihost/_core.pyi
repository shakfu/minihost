...
from typing import Any, Callable, ClassVar, Union, overload
...
class AudioBuffer:
    ...
    # Indexing with two ints returns a scalar sample; any slice in the key
    # returns a sub-buffer.
    @overload
    def __getitem__(self, key: tuple[int, int]) -> float: ...
    @overload
    def __getitem__(self, key: tuple[Any, Any]) -> "AudioBuffer": ...
    ...
    def reverse(self, start: int | None = None, count: int | None = None) -> None: ...
    def reverse_channel(
...
class AudioBufferD:
    """Planar float64 audio buffer (the float64 twin of AudioBuffer)."""
    ...
    def __getitem__(self, key: tuple[int, int]) -> float: ...
    @overload
    def __getitem__(self, key: tuple[Any, Any]) -> "AudioBufferD": ...
    def __setitem__(self, key: tuple[Any, Any], value: Any) -> None: ...
    def clear(self, start: int | None = None, count: int | None = None) -> None: ...
    def apply_gain(self, gain: float) -> None: ...
    def apply_gain_ramp(
    ...
    def copy(self) -> "AudioBufferD": ...
    def as_ndarray(self) -> NDArray[np.float64]: ...
    @classmethod
    def from_numpy(cls, array: Any) -> "AudioBufferD": ...
...
class Plugin:
    ...
    def morph_capture(self) -> list[float]:
        """Snapshot every parameter's current normalized value."""
        ...
    def morph_apply(self, values: list[float]) -> None:
        """Apply a snapshot (clamped to [0, 1]); ValueError on length mismatch."""
        ...
    def morph(self, a: list[float], b: list[float], t: float) -> list[float]:
        """Interpolate snapshots a and b at blend t, apply, and return it."""
        ...
...
class PluginBus:
    """Parallel-branches-summed plugin routing (a mix bus)."""
    ...
    def close(self) -> None: ...
    def __enter__(self) -> "PluginBus": ...
    def __exit__(self, *args: object) -> None: ...

class PluginGraph:
    """General-DAG executor: arbitrary node-to-node audio and MIDI routing."""

    def __init__(self, max_block_size: int, sample_rate: float) -> None: ...
    def add_plugin(self, plugin: "Plugin") -> int: ...
    def add_input(self, channels: int) -> int: ...
    def add_output(self, channels: int) -> int: ...
    def add_mix(self, num_inputs: int, channels: int) -> int: ...
    def add_pick_channel(self, in_channels: int, channel_index: int) -> int: ...
    def add_merge_channels(self, out_channels: int) -> int: ...
    def add_midi_processor(self, params: dict[str, Any]) -> int: ...
    def set_midi_processor_params(
        self, node_id: int, params: dict[str, Any]
    ) -> None: ...
    def add_midi_merge(self, num_inputs: int) -> int: ...
    def connect_midi_port(self, src: int, dst: int, dst_port: int) -> None: ...
    def add_midi_input(self) -> int: ...
    def add_midi_output(self) -> int: ...
    def connect(self, src: int, dst: int, dst_port: int = 0) -> None: ...
    def set_mix_gain(self, mix_node: int, input_index: int, gain: float) -> None: ...
    def connect_midi(self, src: int, dst: int) -> None: ...
    def set_midi_input_events(
        self, node_id: int, events: list[tuple[int, int, int, int]]
    ) -> None: ...
    def get_midi_output_events(
        self, node_id: int
    ) -> list[tuple[int, int, int, int]]: ...
    ...
    def render_block(
        ...
        outputs: list[AudioInput],
        nframes: int,
    ) -> None: ...
    def set_node_automation(
        self, node_id: int, changes: list[tuple[int, int, float]]
    ) -> None: ...
    @property
...
class AudioDevice:
    ...
    def transport_set_bpm(self, bpm: float) -> None: ...
    def transport_set_time_sig(self, numerator: int, denominator: int) -> None: ...
    def transport_set_position(self, position_samples: int) -> None: ...
    def transport_set_loop(
        self, enabled: bool, start_samples: int = 0, end_samples: int = 0
    ) -> None: ...
    def transport_set_recording(self, recording: bool) -> None: ...
    @property
    def transport(self) -> dict[str, Any] | None: ...
    def set_slot_name(self, slot_index: int, name: str | None) -> None: ...
    def slot_name(self, slot_index: int) -> str | None: ...
    def connect_osc(self, port: int = 0) -> None: ...
    def disconnect_osc(self) -> None: ...
    @property
    def osc_port(self) -> int: ...
    def send_param(
        self, param_index: int, value: float, plugin_index: int = 0
    ) -> None: ...
    def send_param_control(
        self, param_index: int, value: float, plugin_index: int = 0
    ) -> None: ...
...
def osc_address_matches(pattern: str, address: str) -> bool: ...
def osc_is_valid_address(address: str) -> bool: ...

class OscServer:
    @staticmethod
    def open(
        port: int, callback: Callable[[str, list[float]], None]
    ) -> "OscServer": ...
    @property
...
class OscClient:
    ...
    def send(
        self,
        address: str,
        value: float | int | bool | str | Sequence[float] | None = None,
    ) -> None: ...
    def close(self) -> None: ...
...
def _message_thread_poll() -> int: ...
def _message_thread_shutdown() -> None:
...
def audio_write(
    ...
    sample_rate: int,
    bit_depth: int = 24,
    bwf: dict[str, Any] | None = None,
) -> None:
    """Write audio data to a WAV or FLAC file. Data shape: (channels, frames).

    Pass ``bwf=dict(...)`` to embed Broadcast Wave (bext) metadata (WAV only).
    """
    ...
