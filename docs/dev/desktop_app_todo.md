...
- [ ] **Autosave / crash recovery.** Periodically snapshot the working `ProjectDocument` to a sidecar file; on next launch, offer to reopen the last session. Bounds a crash's blast radius to a few seconds of unsaved canvas edits. Pre-release.

- [ ] **Document the in-process limitation.** User-facing note (README + in-app About / first-run) that a misbehaving plugin can crash the app and lose unsaved edits. Pre-release.
