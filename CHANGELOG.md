...
- **MIDI output no longer enters libremidi from the audio callback.** The callback sent each generated event with `mh_midi_out_send`, which may lock, allocate or issue a write; a slow backend was an underrun. Events now go into the SPSC ring that was allocated for this and never used, and a pump thread sends them, applying the status-byte length from 0.8.1. A full ring drops the event and counts it rather than blocking.

- **`process` handed the plugin fewer input channels than it reads.** Both CLIs sized the input pointer table from the input file, while `mh_process*` copies `MH_Info.num_input_ch` pointers out of it -- the count the plugin negotiated, which can be larger than the one requested. Renoise Redux takes stereo in and 24 out whatever `mh_open` asks for, so a mono file left the second pointer past the end of the table: `minihost_cpp` segfaulted in the copy, and `minihost_c`, whose tables are fixed-size stack arrays, read an uninitialised slot and rendered whatever was in it. The table is now as wide as the plugin reads, with the channels the file does not supply repeating its last -- so a mono file into a stereo-only plugin is centred, not hard left -- and the sidechain table likewise against `mh_get_sidechain_channels`. `minihost_c` now refuses more than the 32 channels its tables hold rather than passing a short one.
...
- **A plugin's latency, parameter-info, program and I/O changes never reached the host.** A VST3 reports one by calling `restartComponent`, which JUCE's host side defers with `AsyncUpdater`; the plugin thread created a `MessageManager` but never delivered anything queued on it, so those notifications sat there until process teardown. Set a plugin's latency mid-session and `latency_samples` kept reporting the old value indefinitely -- and a deferred callback that finally ran during teardown, against objects already destroyed, is where the two crashes below came from.
  ...
  The macOS pump runs one private run-loop mode that holds nothing but JUCE's queue source. Running `kCFRunLoopDefaultMode` called out to every other main-loop client in the process instead: AudioToolbox's `CAEventReceiver` timer throws `std::bad_function_call` from its own ABI-tagged libc++, a type no `catch` in this binary matches, so `poll_callbacks()` raised `SystemError: exception could not be translated`, and sometimes segfaulted rather than threw. A common mode is no narrower: CoreFoundation drains the main dispatch queue whenever the running mode is one, so `dispatch_async(dispatch_get_main_queue(), ...)` from any framework would still run inside the call.

  JUCE registers that source for `kCFRunLoopCommonModes` only, so `scripts/download_juce.py` now patches `juce_MessageQueue_mac.h` to add it to the private mode as well; the configure step fails if the patch is missing, because an unpatched tree builds and links and then delivers nothing. A plugin's own bundled JUCE is unpatched, so a JUCE plugin's internal message queue is not pumped as a side effect of `poll_callbacks()`, which is also how it behaved before the pump existed.

- **A thread-affine call no longer lets a plugin's exception out of the C API.** `runOnMsg` rethrew whatever the plugin threw on the message thread, so it crossed `extern "C"` and reached a caller that could not name it -- a Python caller got nanobind's "exception could not be translated" instead of the `RuntimeError` the binding documents. It need not even be a C++ exception: Renoise Redux builds an `NSWindow` while parsing a state chunk, and AppKit raises an `NSException` because minihost's message thread is not the process's main thread. The call now reports failure, which is what the header has always specified.
