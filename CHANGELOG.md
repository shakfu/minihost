...
- **`AudioBuffer` class** (`minihost.AudioBuffer`) -- planar float32 audio container, stdlib-only (no numpy required), backed by `juce::AudioBuffer<float>` via a thin C++ wrapper that enforces contiguous memory by construction. Exposes the DLPack and `__array__` protocols so instances can be passed directly to `Plugin.process` / `PluginChain.process` / `write_audio` / `numpy.asarray` without an explicit `.array` / `.numpy()` conversion. Numpy-style 2-axis indexing supported (`buf[ch, fr_slice]`), with documented limits: strided slices, fancy indexing, boolean indexing, and Ellipsis raise `TypeError` directing the user to `.numpy()` for those. JUCE-backed DSP ops `clear`, `apply_gain`, `magnitude`, `copy` exposed; more JUCE ops queued in TODO. Zero-initialized on construction.
