...
- [ ] **Should numpy be a hard dependency?** -- currently declared in `pyproject.toml` (`numpy>=1.20`) and imported unconditionally at the top of `audio_io.py`, so any user of the package pays the install + import cost. Consider whether numpy could be moved to an optional `[numpy]` extra or replaced with stdlib types at the API boundary. Things to investigate: (a) `nb::ndarray<float, c_contig>` accepts any DLPack/buffer-protocol producer, so the C bindings don't strictly require numpy -- a `bytes`/`bytearray`/`array.array('f')` view would work; (b) `read_audio` / `write_audio` / `resample` currently return/accept `np.ndarray`, so a numpy-free path needs an alternative shape ((channels, frames) tuple of buffers? a thin `AudioBuffer` dataclass?); (c) `render.py` uses numpy for slicing, `np.zeros`, `np.concatenate`, `np.max(np.abs(...))` for auto-tail -- these would need stdlib equivalents or a small in-package mini-array. Trade-off: numpy is so pervasive in audio/scientific Python that making it optional may not buy much for typical users, but it would let minimal embedders (e.g. CLI use of `minihost.Plugin` for state inspection) skip a 20+ MB install. Decide the question, then either remove numpy from `dependencies` and add it as `optional-dependencies.numpy = ["numpy>=1.20"]`, or document explicitly that numpy is intentional.
